...
public class Omok {
    ...
    //오목 확인 - 배치된 스톤을 기준으로 확인
    static int search(Board board, Player player, int col, int row) {
    	int[] count = new int[4];
    	int count0 = 0; //33확인 count
    	int count1 = 0; //가로 방향 확인(-)
        int count2 = 0; //세로 방향 확인(ㅣ)
        int count3 = 0; //대각선 방향 확인(/)
        int count4 = 0; //대각선 방향 확인(\)
        int end = 0;
    	...
    	//4개 방향 중 4개가 배치된 경우가 있다면 승리
    	if(count1 == 4 || count2 == 4 || count3 == 4 || count4 == 4) {
    		end = 1;
    	//4개 방향 중 4개 이상 배치된 경우가 있다면 5목 이상이므로 배치할 수 없음
    	} else if(count1 > 4 || count2 > 4 || count3 > 4 || count4 > 4) {
    		end = 2;
    	//count0은 33을 확인하기 위함이므로 주변에 2개 이상 배치된 스톤이 있다면 33으로 배치할 수 없음
    	} else if(count0 >= 2) {
    		end = 3;
    	}
		return end;
